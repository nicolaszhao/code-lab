...
(function ($, window) {
  if (!$.jqDock) {
    //can't see why it should be, but it doesn't hurt to check
    var ONE = 1, //cheat to get past JSLint objecting to things like var i_want_a_number = 1 * var_of_unknown_type;
      ...
      VERTHORZ = {
        //note : lead and trail are indexes into TRBL
      ...
      KEEP_PROPORTION = function (el, dim, verthorz, cap) {
        ...
        r[invwh] = setAbsolute
          ? Math.round((dim * el[invwh]) / el[verthorz.wh])
          : el[invwh];
        return r;
      },
      ...
      CONVERT_LISTS = function () {
        ...
        rtn = $('<div/>')
          ...
          .data(
            ...
            $.extend(
              {},
              original.data('jqdock'),
              NAMESPACED_DATA(original.data()),
            ),
          )
        ...
        convertables.each(function () {
          $(this)
            .data(DATA_NAMES[2], $('<span/>').insertBefore(this)[0])
            .appendTo(rtn);
        });
      ...
      IMAGE_ONLOAD = function (ev) {
        ...
        if (Dock) {
          ...
          if (op.sizeMax) {
            ...
            $.extend(
              elData,
              KEEP_PROPORTION(this, op.sizeMax, VERTHORZ[VERTHORZ[op.vh].inv]),
              op.capSizing,
            );
          } else {
          ...
          if (!--Dock.Load) {
            ...
            WIN_TIMEOUT(function () {
              $.jqdock.init(data.menu);
            }, 0);
          }
        }
      },
          ...
          if (relxy >= 0) {
            ...
            if (ab < op.distance) {
              newFinal =
                el[wh] -
                Math.round(
                  ((el[wh] - el.Initial) * Math.pow(ab, op.coefficient)) /
                    op.attenuation,
                );
            }
      ...
      CHANGE_SIZE = function (Dock, idx, dim, force) {
        ...
        if (force || el.Major !== dim) {
          ...
          bdr =
            $.boxModel || op.vh === 'v'
              ? 0
              : dockBorders[VH.lead] + dockBorders[VH.trail];
          ...
          if (i > 1) {
            //set bottom or right margin
            css['margin' + TRBL[invVH.trail]] = diff;
          } else if (i) {
            //set top & bottom or left & right margins
            ...
            //set top or left margin (op.align = 'bottom' or 'right')
          ...
          if (dim !== el.Major || (force && !idx)) {
            if (op.flow) {
              ...
              yard
                ...
                [
                  VH.wh
                ](Dock.Spread + dockBorders[VH.lead] + dockBorders[VH.trail]);
            }
          ...
          if (!op.flow) {
            ...
            yard.css(
              VH.tl,
              Math.floor(Math.max(0, (Dock[VH.wh] - Dock.Spread) / 2)),
            );
          }
          ...
          if (Dock.OnDock) {
            ...
            if (currentEl && labels) {
              ...
              if (label.mc) {
                ...
                for (i in VERTHORZ) {
                  ...
                  for (j in { lead: 1, trail: 1 }) {
                    label[i] += AS_INTEGER(
                      labelElem.css('padding' + TRBL[VERTHORZ[i][j]]),
                    );
                  }
                }
              }
              ...
              if (labels.charAt(0) === 'm') {
                labelElem.css({
                  top: Math.floor((currentEl[AXES[invVH.xy]] - label.v) / 2),
                });
              }
              if (labels.charAt(1) === 'c') {
                labelElem.css({
                  left: Math.floor((currentEl[AXES[VH.xy]] - label.h) / 2),
                });
              }
            }
            if (!Dock.Stamp) {
      ...
      FACTOR_SIZES = function (Dock) {
        ...
        if (lapse >= op.step) {
          //only if Opts.step ms have passed since last mouseenter/leave
          stepsLeft = (op.duration - lapse) / op.step;
          for (i = 0; i < Dock.ElCt; i++) {
            //must go through the elements in logical order
            el = Dock.Elem[i];
            sz = el.Final - el.Major;
            sz =
              sz && stepsLeft > 1
                ? el.Major + Math[sz < 0 ? 'floor' : 'ceil'](sz / stepsLeft)
                : el.Final;
          ...
          if (Dock.Spread > Dock[VH.wh]) {
            Dock.Yard.parent()[VH.wh](
              Dock.Spread + Dock.Border[VH.lead] + Dock.Border[VH.trail],
            );
      ...
      IN_DOCK = function (Dock, relxy) {
        ...
        if (Dock.OnDock && !Dock.Stamp) {
          ...
          while (i && el[i - 1].Major === el[i - 1].Final) {
            --i;
          }
          if (!i) {
            LABEL_SHOW(Dock, 1); //show
            ...
            FACTOR_SIZES(Dock);
            //set Indock timer...
            Dock[TIMERS[2]] = WIN_TIMEOUT(function () {
              IN_DOCK(Dock, relxy);
            }, Dock.Opts.step);
          }
        }
      ...
      OFF_DOCK = function (Dock, noIdle) {
        var el = Dock.Elem,
          i = el.length;
        if (!Dock.OnDock) {
          while (i && el[i - 1].Major <= el[i - 1].Initial) {
            --i;
          }
          ...
          if (!i) {
            ...
            FACTOR_SIZES(Dock);
            //set Offdock timer...
            Dock[TIMERS[4]] = WIN_TIMEOUT(function () {
              OFF_DOCK(Dock, noIdle);
            }, Dock.Opts.step);
          }
        }
      },
      ...
      OVER_DOCK = function (Dock, relxy) {
        ...
        if (Dock.OnDock) {
          ...
          if (!i || !Dock.Stamp) {
            //complete, or beyond 'duration'
            Dock.Stamp = 0;
            IN_DOCK(Dock, relxy);
          } else {
            FACTOR_SIZES(Dock);
            //set Overdock timer...
            Dock[TIMERS[3]] = WIN_TIMEOUT(function () {
              OVER_DOCK(Dock, relxy);
            }, Dock.Opts.step);
          }
        }
      },
      ...
      DO_MOUSE = function (etype, Dock, idx, fake) {
        ...
        if (etype === 1) {
          if (idx !== Dock.Current) {
            //mousemove from one item onto another
            LABEL_SHOW(Dock); //hide
            Dock.Current = idx;
          }
          IN_DOCK(Dock);
        ...
        if (Dock) {
          if (Dock.Asleep) {
            ...
            if (ev.type === MOUSEEVENTS[2]) {
              ...
              if (ev.type === MOUSEEVENTS[1]) {
                //=mousemove
                if (idx < 0) {
                  if (onDock && Dock.Current >= 0) {
                    //off of current
                    doMse = 2; //mouseleave
                  }
                } else if (!onDock || Dock.Current < 0) {
                  //instant re-entry or no current
                  doMse = 0; //mouseenter
                } else {
                  //change of current or moving within current
                  doMse = 1; //mousemove
                }
              } else if (idx >= 0 && !onDock) {
                //mouseenter...
                doMse = 0; //mouseenter
              }
            }
    ...
    $.jqdock = $.jqDock = (function () {
      return {
        version: '2.0.2',
        defaults: {
          //can be set at runtime, per menu
        ...
        init: function (menu) {
          ...
          Menu.children().each(function (i, kid) {
            var wrap = (Dock.Elem[i].Wrap = $(kid)
              .wrap(vanillaClone.clone().append(vanillaClone.clone()))
              .parent());
            if (op.vh === 'h') {
          ...
          while (i < numItems) {
            ...
            Dock[invVH.wh] = Math.max(
              Dock[invVH.wh],
              op.size + upad[invVH.lead] + upad[invVH.trail],
            );
          ...
          Menu.wrapInner(
            vanillaDiv.addClass(CLASSES[6]).append(
              //nb: outer is .jqDockWrap
              //						vanillaClone.attr({id:'jqDock', 'class':'jqDock', style:''}).css({
              vanillaClone
                .attr({ id: Dock.Id, class: CLASSES[8], style: '' })
                .css({
                  //nb: inner is #jqDockN.jqDock
                  position: 'absolute',
                  top: 0,
                  left: 0,
                  padding: 0,
          ...
          while (i < numItems) {
            ...
            label.el = $('<div/>')
              ...
              .addClass(CLASSES[3] + ' ' + CLASSES[3] + el.Link)
            ...
            if (labelText !== false) {
              ...
              $('<div/>')
                .addClass(CLASSES[3] + 'Text')
                .html(labelText.toString())
                .appendTo(label.el);
            }
          ...
          yard
            .on(MOUSEEVENTS.join(' '), DOTCLASS[1], MOUSE_HANDLER)
            .find('*')
            .css({ filter: 'inherit' });
          ...
          if (!Dock.Asleep) {
            if (fadeLayer) {
              ...
              Dock.Asleep = !!$(DOTCLASS[8] + ',' + DOTCLASS[6], el)
                .addClass(CLASSES[4])
                .css({ filter: 'inherit' });
              el.css({ opacity: 0 });
              REVEAL_MENU(Menu);
              el.animate({ opacity: 1 }, op.fadeIn, function () {
                ...
                DATA(Menu).Asleep = !$(DOTCLASS[4], this)
                  .add(this)
                  .css({ filter: '' })
    ...
    $.fn.jqdock = $.fn.jqDock = function (opts) {
       ...
       ***************************************************************************************************/
      if (opts === 'nudge' || opts === 'idle' || opts === 'freeze') {
        //alternate usage 3 (nudge/idle/freeze)
        rtn.filter(DOTCLASS[0]).each(function () {
          //only runs on an original menu element that has been docked (.jqDocked)
        ...
        rtn = rtn.map(function () {
          ...
          if (Self.length && Dock) {
            ...
            for (i = 0; i < Dock.ElCt; i++) {
              ...
              if (!el.Orig.i.style) {
                //if there was no inline style, might as well remove the style attribute
              ...
              if (el.Link === 'Link') {
                ...
                item.attr(el.Orig.a);
                if (!el.Orig.a.style) {
                  //if there was no inline style, might as well remove it
                  item.removeAttr('style');
                }
              }
            ...
            if (convertedList.el) {
              Self.empty().remove();
              el = $(convertedList.el)
                .removeData(DATA_NAMES[1])
                .attr(convertedList.attr);
        ...
        rtn.each(function () {
          ...
          if (Dock) {
            ...
            if (!Dock.Frozen) {
              Dock.Frozen = Dock.Asleep = !!Menu.trigger('docksleep', [
                'freeze',
              ]);
            }
        ...
        rtn =
          (Dock && isImage
            ? Dock.Elem[ITEM_INDEX(MENU_OR_ITEM(rtn, 1))]
            : Dock) || null;
        /***************************************************************************************************
         * ALTERNATE:
         ...
         ***************************************************************************************************/
      } else if (rtn.length && !rtn.not('img').length) {
        //images only!!
        rtn.each(function (n, el) {
          ...
          if (item) {
            ...
            for (v in { src: 1, altsrc: 1 }) {
              if (opts[v]) {
                str = (
                  $.isFunction(opts[v]) ? opts[v].call(el, item[v], v) : opts[v]
                ).toString();
                if (item[v] !== str) {
                  item[v] = str;
        ...
        rtn = rtn
          ...
          .filter(function () {
            ...
            return (
              !MENU_OR_ITEM(this).length &&
              kids.length &&
              !kids.not('img').filter(function () {
                //returns true for everything EXCEPT an anchor containing just 1 image...
                return (
                  $(this).children('img').parent('a').children().length !== 1
                );
              }).length
            );
          })
          .each(function () {
            var Menu = $(this).addClass(CLASSES[0]),
              ...
              Dock = {
                ...
                ToFro: [
                  //a pain, but needed to prevent possible oscillation around a stationary cursor on the dock (see SET_SIZES)...
...
*/
            ...
            op.fadeLayer = op.fadeIn
              ? { dock: 1, wrap: 1 }[op.fadeLayer]
                ? op.fadeLayer
                : 'menu'
              : '';
            ...
            images.each(function (n, el) {
              ...
              altsrc =
                (op.source ? op.source.call(el, n) : '') || //image path, large
                jself.data('jqdockAltsrc') ||
                '' ||
                (/\.(gif|jpg|jpeg|png)$/i.test(origImg.alt || '')
                  ? origImg.alt
                  : '') ||
                origImg.src;
              Dock.Elem[n] = {
                ...
                Orig: {
                  i: $.extend({}, origImg),
                  a: {
                    title: origAnchorTitle,
                    style: linkParent.attr('style') || '',
                  },
                },
                Label: {
...
*/
              ...
              i = 4;
              while (i--) {
                Dock.Elem[n].Pad[i] = AS_INTEGER(
                  jself.css('padding' + TRBL[i]),
                );
              }
              ...
              if (op.loader ? op.loader === 'jquery' : $.jqdock.useJqLoader) {
                //jQuery method...
                $('<img>')
                  .on('load', { idx: n, menu: Menu[0] }, IMAGE_ONLOAD)
                  .attr({ src: altsrc });
              } else {
                //native 'new Image()' method...
                pre = new Image();
                pre.onload = function () {
